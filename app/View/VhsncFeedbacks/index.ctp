...
<div class="actions"><h2><?php echo __('VHSNC Feedback Details'); ?></h2></div>
<?php $menu= $this->Session->read('User.mainmenu');
      $sessionval=$this->Session->read('User.type');
     
?>
<div class="btn-group">
<?php echo $this->Html->link(__('New VHSNC Feedback'), array('action' => 'add'),array('class' => 'btn btn-primary')); ?>
<?php echo $this->Html->link(__('List VHSNC Meeting'), array('controller' => 'vhsncMeetings','action' => 'index'),array('class' => 'btn btn-primary')); ?>
</div>

<div class="panel panel-default">
<div class="panel-body">
<div class="row">
<form id="mastersearch" mathod="get" action="<?=SITE_PATH?>vhsncFeedbacks/">
<?php /*?> <div class="txt_one">SEARCH</div><?php */?>
<div class="col-sm-3"><input type="text" name="search_key" placeholder="By Name,Mobile,Response,Status" class="form-control" value="<? if(isset($this->params->query['search_key'])){ echo trim($this->params->query['search_key']); }?>"/></div>
...
<div class="col-sm-1"><input type="button" name="reset" value="Reset" class="btn btn-warning btn-block" onclick="window.location.href='<?=SITE_PATH?>vhsncFeedbacks/'"/></div>
<div class="col-sm-1" style="float:right; margin-top:15px;"><input type="button" name="reset" value="Export" class="btn btn-block export" onclick="window.open('<?=SITE_PATH?>vhsncFeedbacks/export/?<?=$_SERVER['QUERY_STRING']?>','abhay','scrollbars=1,width=1400,height=650,left=100,top=50')"/></div>
...
<tr>
<td><?php echo h($vhsncafc['VhsncFeedback']['id']); ?></td>
...
<td><?php echo h(date('d-m-Y',strtotime($vhsncafc['VhsncFeedback']['meeting_date']))); ?></td>
<td><?php  echo h(ucfirst($vhsncafc['VhsncFeedback']['name'])) ?></td>
<td><?php  echo h(ucfirst($vhsncafc['VhsncFeedback']['mobile'])) ?></td>
<!--<td class="question"><?php
///if($vhsncafc['VhsncFeedback']['question']!='') {
...
<td class="actions">
<a href="javascript:void(0)" class="more btn btn-success" data-toggle="modal" data-target="#myModal" data-id="<?=$vhsncafc['VhsncFeedback']['id']?>">More</a>
<?php 
...
?>
<?php //echo $this->Html->link(__('Edit'), array('action' => 'edit', $vhsncafc['VhsncFeedback']['id']),array('class' => 'btn btn-primary')); ?>
<?php } ?>
   <?php
if(in_array($this->request->params['controller'].":delete",$menu)){
  
?>
  <?php //echo $this->Form->postLink(__('Delete'), array('action' => 'delete', $vhsncafc['VhsncFeedback']['id']),array('class' => 'btn btn-danger'), null, __('Are you sure you want to delete # %s?', $vhsncafc['VhsncFeedback']['id'])); ?>
...
$.ajax({url:"<?=SITE_PATH?>/VhsncFeedbacks/view/"+dataid,dataType:"html",success:function(result){
