...
<div class="actions"><h2><?php echo __('AFC Details'); ?></h2></div>
...
?>
<div class="btn-group">
<?php echo $this->Html->link(__('New AFC'), array('action' => 'add'),array('class' => 'btn btn-primary')); ?>
...
<div class="row">
<form id="mastersearch" mathod="get" action="<?=SITE_PATH?>vhsncAfcs/">
<?php /*?> <div class="txt_one">SEARCH</div><?php */?>
<div class="col-sm-3"><input type="text" name="search_key" placeholder="By Member Name,Mobile" class="form-control" value="<? if(isset($this->params->query['search_key'])){ echo trim($this->params->query['search_key']); }?>"/></div>
...
<div class="col-sm-1"><input type="button" name="reset" value="Reset" class="btn btn-warning btn-block" onclick="window.location.href='<?=SITE_PATH?>vhsncAfcs/'"/></div>
<div class="col-sm-1" style="float:right; margin-top:15px;"><input type="button" name="reset" value="Export" class="btn btn-block export" onclick="window.open('<?=SITE_PATH?>vhsncAfcs/export/?<?=$_SERVER['QUERY_STRING']?>','abhay','scrollbars=1,width=1400,height=650,left=100,top=50')"/></div>
...
<th><?php //echo $this->Paginator->sort('Total Member'); ?>Total Members</th>
...
<tr>
<td><?php echo h($vhsncafc['VhsncAfc']['id']); ?></td>
<td><?php echo h(ucfirst($vhsncafc['Ngo']['name_of_ngo'])); ?></td>
...
<td><?php echo h(ucfirst($vhsncafc['Village']['name'])); ?></td>
<td><?php 
$questionlist=$this->requestAction(array("controller"=>"vhsncAfcs","action"=>"gettotal",$vhsncafc['VhsncAfc']['village'])); 
echo  count($questionlist);
//echo h(ucfirst($vhsncafc['Village']['name'])); ?></td>
<td><a href="javascript:void(0)" class="more btn btn-success" data-toggle="modal" data-target="#myModal" data-id="<?=$vhsncafc['VhsncAfc']['village']?>">More</a></td>
...
$.ajax({url:"<?=SITE_PATH?>vhsncAfcs/view/"+dataid,dataType:"html",success:function(result){
