...
<div class="actions"><h2><?php echo __('VHSND Details'); ?></h2></div>
...
?>
<div class="btn-group">
<?php echo $this->Html->link(__('New VHSND'), array('action' => 'add'),array('class' => 'btn btn-primary')); ?>
...
<div class="row">
<form id="mastersearch" mathod="get" action="<?=SITE_PATH?>vhsnds/">
<?php /*?> <div class="txt_one">SEARCH</div><?php */?>
<div class="col-sm-3"><input type="text" name="search_key" placeholder="By Availability of IT,HB,Weight,Height..." class="form-control" value="<? if(isset($this->params->query['search_key'])){ echo trim($this->params->query['search_key']); }?>"/></div>
...
<div class="col-sm-1"><input type="button" name="reset" value="Reset" class="btn btn-warning btn-block" onclick="window.location.href='<?=SITE_PATH?>vhsnds/'"/></div>
<div class="col-sm-1" style="float:right;"><input type="button" name="reset" value="Export" class="btn btn-block export" onclick="window.open('<?=SITE_PATH?>vhsnds/export/?<?=$_SERVER['QUERY_STRING']?>','abhay','scrollbars=1,width=1400,height=650,left=100,top=50')"/></div>
...
<th><?php echo $this->Paginator->sort('awc_code'); ?></th>
...
<?php foreach ($vhsnds as $vhsnd): ?>
<tr>
<td><?php echo h($vhsnd['Vhsnd']['id']); ?></td>
<td><?php echo h(ucfirst($vhsnd['City']['name'])); ?></td>
<td><?php echo h(ucfirst($vhsnd['Block']['name'])); ?></td>
<td><?php echo h(ucfirst($vhsnd['Panchayat']['name'])); ?></td>
<td><?php echo h(ucfirst($vhsnd['Village']['name'])); ?></td>
<td><?php echo h(ucfirst($vhsnd['Ward']['name'])); ?></td>
<td><?php echo h(ucfirst($vhsnd['Geographical']['awc_code'])); ?></td>
<td><?php echo date('d-m-Y',strtotime($vhsnd['Vhsnd']['visit_date'])); ?></td>
<td class="actions">
<a href="javascript:void(0)" class="more btn btn-success" data-toggle="modal" data-target="#myModal" data-id="<?=$vhsnd['Vhsnd']['id']?>">More</a>
<?php 
if($sessionval=='regular') {
?>
<?php

  if($sessionrole=='CC' && $vhsnd['Vhsnd']['visit_date']==date('Y-m-d')) {
      if(in_array($this->request->params['controller'].":edit",$menu) && $vhsnd['Vhsnd']['updated']==0){
   echo $this->Html->link(__('Edit'), array('action' => 'edit', $vhsnd['Vhsnd']['id']),array('class' => 'btn btn-primary'));
       }
    
  }else if($sessionrole=='BPC') {
        if(in_array($this->request->params['controller'].":edit",$menu) && $vhsnd['Vhsnd']['updated']==0){
   echo $this->Html->link(__('Edit'), array('action' => 'edit', $vhsnd['Vhsnd']['id']),array('class' => 'btn btn-primary'));
       }
  }
  else if($sessionrole=='DPO') {
    echo $this->Html->link(__('Edit'), array('action' => 'edit', $vhsnd['Vhsnd']['id']),array('class' => 'btn btn-primary'));
...
?>
  <?php echo $this->Form->postLink(__('Delete'), array('action' => 'delete', $vhsnd['Vhsnd']['id']),array('class' => 'btn btn-danger'), null, __('Are you sure you want to delete # %s?', $vhsnd['Vhsnd']['id'])); ?>

<?php } ?>
<?php } else {?>
    <?php echo $this->Html->link(__('Edit'), array('action' => 'edit', $vhsnd['Vhsnd']['id']),array('class' => 'btn btn-primary')); ?>
<?php echo $this->Form->postLink(__('Delete'), array('action' => 'delete', $vhsnd['Vhsnd']['id']),array('class' => 'btn btn-danger'), null, __('Are you sure you want to delete # %s?', $vhsnd['Vhsnd']['id'])); ?>
<?php } ?>
</td>
</tr>
...
$.ajax({url:"<?=SITE_PATH?>vhsnds/view/"+dataid,dataType:"html",success:function(result){
