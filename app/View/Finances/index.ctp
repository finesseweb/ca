...
<div class="actions"><h2><?php echo __('Invoice Details'); ?></h2></div>
<div class="btn-group">
<?php echo $this->Html->link(__('Add New Invoice'), array('action' => 'add'),array('class' => 'btn btn-primary')); ?>
...
<div class="row">
<form id="mastersearch" mathod="get" action="<?=SITE_PATH?>finances/">
...
<tr>
<td><?php echo h($financial['Finance']['id']); ?></td>
<td><?php echo h(ucfirst($financial['CompanyDetail']['name_of_company'])); ?></td>
<td><?php echo $financial['Period']['financial_year']; ?></td>
<td><?php echo $financial['ClientDetail']['company_name']; ?></td>
<td><?php echo $financial['Finance']['invoice_number']; ?></td>
<td><?php echo h(number_format($financial['Finance']['current_total_amount'])); ?>.00</td>
<td><?php echo h(date('d-m-Y',strtotime($financial['Finance']['invoice_date']))); ?></td>
<td><?php 
if($financial['Finance']['status']=='active'){
echo h(ucfirst($financial['Finance']['status'])); 
}
else {
...
<td class="actions">
<a href="javascript:void(0)" class="more btn btn-success" data-toggle="modal" data-target="#myModal" data-id="<?=$financial['Finance']['company']?>,<?=$financial['Finance']['company_name']?>,<?=$financial['Finance']['slip']?>">More</a>
<a  class="more btn btn-success" onclick="window.open('<?=SITE_PATH?>finances/report/?com=<?=$financial['Finance']['company']?>&cl=<?=$financial['Finance']['company_name']?>&slip=<?=$financial['Finance']['slip']?>','abhay','scrollbars=1,width=1400,height=650,left=100,top=50')"/>Print</a>
<a  class="more btn btn-success" onclick="window.open('<?=SITE_PATH?>finances/testpdf/?com=<?=$financial['Finance']['company']?>&cl=<?=$financial['Finance']['company_name']?>&slip=<?=$financial['Finance']['slip']?>','abhay','scrollbars=1,width=1400,height=650,left=100,top=50')"/>Print</a>
    <?php 

//echo $this->Html->link(__('PDF'), array('action' => 'view_pdf', 'ext' => 'pdf', $post['Post']['id']));
//echo $this->Html->link(__('Edit'), array('action' => 'edit', $financial['Finance']['id']),array('class' => 'btn btn-primary')); ?>
<?php 
if($financial['Finance']['status']=='active'){
echo $this->Form->postLink(__('Cancel'), array('action' => 'delete', $financial['Finance']['slip']),array('class' => 'btn btn-danger'), null, __('Are you sure you want to delete # %s?', $financial['Finance']['slip'])); 
} else {
    
 echo $this->Form->postLink(__('Cancel'), array('action' => 'delete', $financial['Finance']['slip']),array('class' => 'btn btn-danger not-active'), null, __('Are you sure you want to delete # %s?', $financial['Finance']['slip'])); 
...
$.ajax({url:"<?=SITE_PATH?>finances/viewdetails/"+dataid,dataType:"html",success:function(result){
