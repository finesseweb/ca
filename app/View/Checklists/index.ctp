...
<div class="actions"><h2><?php echo __('Checklist'); ?></h2></div>
...
<div class="row">
<form id="mastersearch" mathod="get" action="<?=SITE_PATH?>checklists/">
<?php /*?> <div class="txt_one">SEARCH</div><?php */?>
<div class="col-sm-3"><input type="text" name="search_key" placeholder="By Monitor Name" class="form-control" value="<? if(isset($this->params->query['search_key'])){ echo trim($this->params->query['search_key']); }?>"/></div>
...
<div class="col-sm-1"><input type="button" name="reset" value="Reset" class="btn btn-warning btn-block" onclick="window.location.href='<?=SITE_PATH?>checklists/'"/></div>
<div class="col-sm-1" style="float:right; margin-top:15px;"><input type="button" name="reset" value="Export" class="btn btn-block export" onclick="window.open('<?=SITE_PATH?>checklists/export/?<?=$_SERVER['QUERY_STRING']?>','abhay','scrollbars=1,width=1400,height=650,left=100,top=50')"/></div>
...
<th><?php echo $this->Paginator->sort('district_ame'); ?></th>
<th><?php echo $this->Paginator->sort('block_name'); ?></th>
<th><?php echo $this->Paginator->sort('monitoring_date'); ?></th>
<th><?php echo $this->Paginator->sort('name_of_monitor'); ?> </th>
...
<tr>
<td><?php echo h($vhsncafc['Checklist']['id']); ?></td>
...
<td><?php echo h(date('d-m-Y',strtotime($vhsncafc['Checklist']['monitoring_date']))); ?></td>
<td><?php  echo h(ucfirst($vhsncafc['Checklist']['name_of_monitor']));  ?></td>
<td class="actions">
<a href="javascript:void(0)" class="more btn btn-success" data-toggle="modal" data-target="#myModal" data-id="<?=$vhsncafc['Checklist']['id']?>">More</a>
<?php 
...
$.ajax({url:"<?=SITE_PATH?>/Checklists/view/"+dataid,dataType:"html",success:function(result){
