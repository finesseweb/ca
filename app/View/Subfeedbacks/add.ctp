...
<div class="btn-group">
<?php echo $this->Html->link(__('List Questions'), array('action' => 'index'),array('class' => 'btn btn-primary')); ?>
<?php echo $this->Html->link(__('List Feedback Title'), array('controller' => 'feedbacks', 'action' => 'index'),array('class' => 'btn btn-primary')); ?>
<?php echo $this->Html->link(__('New Feedback Title '), array('controller' => 'feedbacks', 'action' => 'add'),array('class' => 'btn btn-primary')); ?>
...
echo $this->Form->input('cat_id',array('class' => 'form-control','label'=>'Title','options'=>array(''=>'--Select--',$financials)));
...
$("#SubfeedbackCategory").change(function(){
var c=$(this).val();
$('#SubfeedbackCatId').html("<option value=''>loading......</option>"); 
$.ajax({url:"<?=SITE_PATH?>feedbacks/gettitles/"+c,success:function(result){$("#SubfeedbackCatId").html(result);}});
