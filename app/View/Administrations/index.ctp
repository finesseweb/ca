...
<h2><?php echo __('Administration Details'); ?></h2>
<div class="btn-group">
<?php echo $this->Html->link(__('New Administrator'), array('action' => 'add'),array('class' => 'btn btn-primary')); ?>
...
<div class="row">
<form id="mastersearch" mathod="get" action="<?=SITE_PATH?>administrations/">
...
<div class="col-sm-1"><input type="button" name="reset" value="Reset" class="btn btn-warning btn-block" onclick="window.location.href='<?=SITE_PATH?>administrations/'"/></div>
...
<th><?php echo $this->Paginator->sort('emailid'); ?></th>
...
<td><?php echo h(ucfirst($administration['Ngo']['name_of_ngo'])); ?></td>
<td><?php echo h(ucfirst($administration['User']['name'])); ?></td>
<td><?php echo h(ucfirst($administration['User']['last_name'])); ?></td>
<td><?php echo h(ucfirst($administration['Designation']['name'])); ?></td>
<td><?php echo h(ucfirst($administration['Administration']['gender'])); ?></td>
<td><?php echo h($administration['Administration']['mobile']); ?></td>
<td><?php echo h($administration['Administration']['emailid']); ?></td>
<td class="actions">
<a href="javascript:void(0)" class="more btn btn-success" data-toggle="modal" data-target="#myModal" data-id="<?=$administration['Administration']['id']?>">More</a>
<?php echo $this->Html->link(__('Edit'), array('action' => 'edit', $administration['Administration']['id']),array('class' => 'btn btn-primary')); ?>
<?php echo $this->Form->postLink(__('Delete'), array('action' => 'delete', $administration['Administration']['id']),array('class' => 'btn btn-danger'), null, __('Are you sure you want to delete # %s?', $administration['Administration']['id'])); ?>
</td>
</tr>
<?php endforeach; ?>
</table>
...
$.ajax({url:"<?=SITE_PATH?>administrations/view/"+dataid,dataType:"html",success:function(result){
