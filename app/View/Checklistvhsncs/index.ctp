...
<h2><?php echo __('Checklist VHSNC'); ?></h2>
<?php $menu= $this->Session->read('User.mainmenu');
      $sessionval=$this->Session->read('User.type');
      
?>
<div class="btn-group">
<?php echo $this->Html->link(__('New Checklist'), array('action' => 'add'),array('class' => 'btn btn-primary')); ?>
...
<div class="row">
<form id="mastersearch" mathod="get" action="<?=SITE_PATH?>checklistvhsncs/">
...
<div class="col-sm-1"><input type="button" name="reset" value="Reset" class="btn btn-warning btn-block" onclick="window.location.href='<?=SITE_PATH?>checklistvhsncs/'"/></div>
<div class="col-sm-1" style="float:right; margin-top:15px;"><input type="button" name="reset" value="export" class="btn btn-block" onclick="window.open('<?=SITE_PATH?>checklistvhsncs/export/?<?=$_SERVER['QUERY_STRING']?>','abhay','scrollbars=1,width=1400,height=650,left=100,top=50')"/></div>
...
<th><?php echo $this->Paginator->sort('block_name'); ?></th>
<th><?php echo $this->Paginator->sort('date'); ?></th>
<th><?php //echo $this->Paginator->sort('Name of Investigator'); ?>Name of Monitor </th>
...
<tr>
<td><?php echo h($vhsncafc['Checklistvhsnc']['id']); ?></td>
...
<td><?php echo h(date('d-m-Y',strtotime($vhsncafc['Checklistvhsnc']['meeting_date']))); ?></td>
<td><?php  echo h(ucfirst($vhsncafc['Checklistvhsnc']['name_of_monitor']));  ?></td>
<td class="actions">
<a href="javascript:void(0)" class="more btn btn-success" data-toggle="modal" data-target="#myModal" data-id="<?=$vhsncafc['Checklistvhsnc']['id']?>">More</a>
<?php 
...
<?php } ?>
<?php }  else {?>
<?php echo $this->Html->link(__('Edit'), array('action' => 'edit', $vhsncafc['Checklistvhsnc']['id']),array('class' => 'btn btn-primary')); ?>
<?php echo $this->Form->postLink(__('Delete'), array('action' => 'delete', $vhsncafc['Checklistvhsnc']['id']),array('class' => 'btn btn-danger'), null, __('Are you sure you want to delete # %s?', $vhsncafc['Checklistvhsnc']['id'])); ?>
...
$.ajax({url:"<?=SITE_PATH?>/checklistvhsncs/view/"+dataid,dataType:"html",success:function(result){
