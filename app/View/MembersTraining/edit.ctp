...
?>
    <div class="col-sm-3"><label>Date of Training <span style="color:red">*</span></label><div class="input-group"><input type="text" name="data[MembersTraining][from_date]" value="<?php echo date('d-m-Y',strtotime($this->request->data['MembersTraining']['from_date']))?>" id="from_date" class="form-control" placeholder="Date From"  required="required"/><span class="input-group-addon">To</span><input type="text" name="data[MembersTraining][to_date]" id="to_date" value="<?php echo date('d-m-Y',strtotime($this->request->data['MembersTraining']['to_date']))?>" class="form-control" placeholder="Date To" /></div></div>
