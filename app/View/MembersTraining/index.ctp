...
<div class="actions"><h2><?php echo __('Members Training Details'); ?></h2></div>
<?php $menu= $this->Session->read('User.mainmenu');
      $sessionval=$this->Session->read('User.type');
      
?>
<div class="btn-group">
<?php echo $this->Html->link(__('New Member Training '), array('action' => 'add'),array('class' => 'btn btn-primary')); ?>
...
<div class="row">
<form id="mastersearch" mathod="get" action="<?=SITE_PATH?>membersTraining/">
<?php /*?> <div class="txt_one">SEARCH</div><?php */?>
<div class="col-sm-3"><input type="text" name="search_key" placeholder="By Mobile,Name,Type" class="form-control" value="<? if(isset($this->params->query['search_key'])){ echo trim($this->params->query['search_key']); }?>"/></div>
...
<div class="col-sm-1"><input type="button" name="reset" value="Reset" class="btn btn-warning btn-block" onclick="window.location.href='<?=SITE_PATH?>membersTraining/'"/></div>
<div class="col-sm-1" style="float:right;"><input type="button" name="reset" value="Export" class="btn btn-block export" onclick="window.open('<?=SITE_PATH?>membersTraining/export/?<?=$_SERVER['QUERY_STRING']?>','abhay','scrollbars=1,width=1400,height=650,left=100,top=50')"/></div>
...
<!--<th><?php //echo $this->Paginator->sort('member_type'); ?></th>-->
<th><?php //echo $this->Paginator->sort('Date of Training'); ?>Date of Training</th>
...
<tr>
    <td><?php echo h($vhsnc['MembersTraining']['id']); ?></td>
...
<!--     <td><?php //echo h(ucfirst($vhsnc['MembersTraining']['members_type'])); ?></td>-->


<td><?php echo h(date('d-m-Y',strtotime($vhsnc['MembersTraining']['from_date']))); ?></td>

<td class="actions">
<a href="javascript:void(0)" class="more btn btn-success" data-toggle="modal" data-target="#myModal" data-id="<?=$vhsnc['MembersTraining']['from_date']?>,<?=$vhsnc['MembersTraining']['panchayat']?>">More</a>
<?php /*?> <?php 
...
$.ajax({url:"<?=SITE_PATH?>membersTraining/viewdetails/"+dataid,dataType:"html",success:function(result){
