...
<div class="actions">
<h2><?php echo __('Facility Details'); ?></h2>
</div>
<div class="btn-group">
<?php echo $this->Html->link(__('New Facility'), array('action' => 'add'),array('class' => 'btn btn-primary')); ?>
...
<div class="row">
<form id="mastersearch" mathod="get" action="<?=SITE_PATH?>facilityDetails/">
<?php /*?> <div class="txt_one">SEARCH</div><?php */?>
<div class="col-sm-3"><input type="text" name="search_key" placeholder="By Doctor name,Facilty Name,ANM name,Facility Type" class="form-control" value="<? if(isset($this->params->query['search_key'])){ echo trim($this->params->query['search_key']); }?>"/></div>
...
<div class="col-sm-1"><input type="button" name="reset" value="Reset" class="btn btn-warning btn-block" onclick="window.location.href='<?=SITE_PATH?>facilityDetails/'"/></div>
<div class="col-sm-1" style="float:right; margin-top:15px;"><input type="button" name="reset" value="Export" class="btn btn-block export" onclick="window.open('<?=SITE_PATH?>facilityDetails/export/?<?=$_SERVER['QUERY_STRING']?>','abhay','scrollbars=1,width=1400,height=650,left=100,top=50')"/></div>
...
<th><?php echo $this->Paginator->sort('health_facility_name'); ?></th>
 <th><?php echo $this->Paginator->sort('facility_type'); ?></th>
  <th><?php echo $this->Paginator->sort('functionality'); ?></th>
<th class="actions"><?php echo __('Actions'); ?></th>
</tr>
<?php foreach ($geographicals as $geographical): ?>
<tr>
<td><?php echo h($geographical['FacilityDetail']['id']); ?></td>
...
<td><?php echo h(ucfirst($geographical['FacilityDetail']['health_facility_name'])); ?></td>
<td><?php echo h(ucfirst($geographical['FacilityDetail']['facility_type'])); ?></td>
<td><?php echo h(ucfirst($geographical['FacilityDetail']['functionality'])); ?></td>
<td class="actions">
<a href="javascript:void(0)" class="more btn btn-success" data-toggle="modal" data-target="#myModal" data-id="<?=$geographical['FacilityDetail']['id']?>">More</a>
<?php echo $this->Html->link(__('Edit'), array('action' => 'edit', $geographical['FacilityDetail']['id']),array('class' => 'btn btn-primary')); ?>
<?php echo $this->Form->postLink(__('Delete'), array('action' => 'delete', $geographical['FacilityDetail']['id']),array('class' => 'btn btn-danger'), null, __('Are you sure you want to delete # %s?', $geographical['FacilityDetail']['id'])); ?>
</td>
</tr>
<?php endforeach; ?>
</table>
...
$.ajax({url:"<?=SITE_PATH?>facilityDetails/view/"+dataid,dataType:"html",success:function(result){
