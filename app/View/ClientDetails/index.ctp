...
<div class="btn-group">
<?php echo $this->Html->link(__('New Client'), array('action' => 'add'),array('class' => 'btn btn-primary')); ?>
...
<div class="row">
<form id="mastersearch" mathod="get" action="<?=SITE_PATH?>client_details/">
<?php /*?> <div class="txt_one">SEARCH</div><?php */?>
<div class="col-sm-3"><input type="text" name="search_key" placeholder="By Client name,phone,email,company" class="form-control" value="<? if(isset($this->params->query['search_key'])){ echo trim($this->params->query['search_key']); }?>"/></div>
...
<div class="col-sm-1"><input type="button" name="reset" value="Reset" class="btn btn-warning btn-block" onclick="window.location.href='<?=SITE_PATH?>geographicals/'"/></div>
<?php //if(CakeSession::read('User.type')==='admin'){?>
<!--<div class="col-sm-1" style="float:right;"><input type="button" name="reset" value="Export" class="btn btn-block export" onclick="window.open('<?=SITE_PATH?>geographicals/export/?<?=$_SERVER['QUERY_STRING']?>','abhay','scrollbars=1,width=1400,height=650,left=100,top=50')"/></div>-->
<?php //} ?>
</form>
</div>
</div>
...
<th><?php echo $this->Paginator->sort('name_of_client'); ?></th>
<th><?php echo $this->Paginator->sort('client_email'); ?></th>
<th><?php echo $this->Paginator->sort('client_phone'); ?></th>
...
<tr>
<td><?php echo h($geographical['ClientDetail']['id']); ?></td>
<td><?php echo h(ucfirst($geographical['ClientDetail']['name_of_client'])); ?></td>
<td><?php echo h(ucfirst($geographical['ClientDetail']['client_email'])); ?></td>
<td><?php echo h(ucfirst($geographical['ClientDetail']['client_phone'])); ?></td>
<td><?php echo h(ucfirst($geographical['ClientDetail']['company_name'])); ?></td>
<td><?php echo h(ucfirst($geographical['ClientDetail']['status'])); ?></td>
<td class="actions">
<a href="javascript:void(0)" class="more btn btn-success" data-toggle="modal" data-target="#myModal" data-id="<?=$geographical['ClientDetail']['id']?>">More</a>
<?php echo $this->Html->link(__('Edit'), array('action' => 'edit', $geographical['ClientDetail']['id']),array('class' => 'btn btn-primary')); ?>
<?php echo $this->Html->link(__('Delete'), array('action' => 'delete', $geographical['ClientDetail']['id']),array('class' => 'btn btn-danger')); ?>
</td>
</tr>
<?php endforeach; ?>
</table>
...
$.ajax({url:"<?=SITE_PATH?>client_details/view/"+dataid,dataType:"html",success:function(result){
