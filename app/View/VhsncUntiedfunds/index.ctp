...
<div class="actions"><h2><?php echo __('VHSNC Expenditure Details'); ?></h2></div>
<?php $menu= $this->Session->read('User.mainmenu');
      $sessionval=$this->Session->read('User.type');
      
?>
<div class="btn-group">
<?php echo $this->Html->link(__('New VHSNC Expenditure'), array('action' => 'add'),array('class' => 'btn btn-primary')); ?>
<?php echo $this->Html->link(__('List VHSNC Untied funds'), array('controller' => 'vhsncUntiedfundDetails','action' => 'index'),array('class' => 'btn btn-primary')); ?>
</div>

<div class="panel panel-default">
<div class="panel-body">
<div class="row">
<form id="mastersearch" mathod="get" action="<?=SITE_PATH?>vhsncUntiedfunds/">
<?php /*?> <div class="txt_one">SEARCH</div><?php */?>
<div class="col-sm-3"><input type="text" name="search_key" placeholder="By Expenditue details, Expenditure Amount" class="form-control" value="<? if(isset($this->params->query['search_key'])){ echo trim($this->params->query['search_key']); }?>"/></div>
...
<div class="col-sm-1"><input type="button" name="reset" value="Reset" class="btn btn-warning btn-block" onclick="window.location.href='<?=SITE_PATH?>vhsncUntiedfunds/'"/></div>
<div class="col-sm-1" style="float:right; margin-top:15px;"><input type="button" name="reset" value="Export" class="btn btn-block export" onclick="window.open('<?=SITE_PATH?>vhsncUntiedfunds/export/?<?=$_SERVER['QUERY_STRING']?>','abhay','scrollbars=1,width=1400,height=650,left=100,top=50')"/></div>
...
<th><?php //echo $this->Paginator->sort('total_expenditure '); ?>Total Expenditure</th>
<th><?php //echo $this->Paginator->sort('balance_amount'); ?>Balance Amount</th>
<th><?php //echo $this->Paginator->sort('balance_amount'); ?>Current Balance</th>
<th class="actions"><?php echo __('Actions'); ?></th>
</tr>
<?php foreach ($untiedfunds as $untiedfund): ?>
<tr>
<td><?php echo h($untiedfund['VhsncUntiedfund']['id']); ?></td>
...
$questionlist=$this->requestAction(array("controller"=>"vhsncConstitutions","action"=>"getname",$untiedfund['VhsncUntiedfund']['panchayat'])); 
                 //print_r($questionlist);
                  echo ucwords($questionlist); ?></td>
<td><?php echo h(date('d-m-Y',strtotime($untiedfund['VhsncUntiedfund']['balance_check_date']))); ?></td>
<td><?php echo h(ucfirst($untiedfund['VhsncUntiedfund']['current_total_amount'])); ?></td>
<td><?php echo h(ucfirst($untiedfund['VhsncUntiedfund']['balance_on_date'])); ?></td>
<td><?php 

$questionlist=$this->requestAction(array("controller"=>"vhsncUntiedfunds","action"=>"getcurrbalance",$untiedfund['VhsncUntiedfund']['vhsnc_name'])); 
                 //print_r($questionlist);
                  echo ucwords($questionlist-$untiedfund['VhsncUntiedfund']['current_total_amount']); ?></td>
<td class="actions">
<a href="javascript:void(0)" class="more btn btn-success" data-toggle="modal" data-target="#myModal" data-id="<?=$untiedfund['VhsncUntiedfund']['panchayat']?>,<?=$untiedfund['VhsncUntiedfund']['expenditure_date']?>">More</a>
<?php /*?><?php 
...
$.ajax({url:"<?=SITE_PATH?>vhsncUntiedfunds/viewdetails/"+dataid,dataType:"html",success:function(result){
