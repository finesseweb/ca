...
<div class="actions"><h2><?php echo __('Daily Report'); ?></h2></div>
...
<?php echo $this->Html->link(__('List Monthly Report'), array('controller' => 'monthlyreports','action' => 'index'),array('class' => 'btn btn-primary')); ?>
...
<div class="row">
<form id="mastersearch" mathod="get" action="<?=SITE_PATH?>commonreports/">
<?php /*?> <div class="txt_one">SEARCH</div><?php */?>
<div class="col-sm-2"><input type="text" name="search_key" placeholder="By Activity" class="form-control" value="<? if(isset($this->params->query['search_key'])){ echo trim($this->params->query['search_key']); }?>"/></div>
...
<div class="col-sm-3"><select name="user" class="form-control" id="user">
...
<div class="col-sm-1"><input type="button" name="reset" value="Reset" class="btn btn-warning btn-block" onclick="window.location.href='<?=SITE_PATH?>commonreports/'"/></div>
<div class="col-sm-1" style="float:right; margin-top:15px;"><input type="button" name="reset" value="Export" class="btn btn-block export" onclick="window.open('<?=SITE_PATH?>commonreports/export/?<?=$_SERVER['QUERY_STRING']?>','abhay','scrollbars=1,width=1400,height=650,left=100,top=50')"/></div>
...
<tr>
<td><?php echo h($vhsncafc['Commonreport']['id']); ?></td>
<td><?php echo h(ucfirst($vhsncafc['City']['name'])); ?></td>
<td><?php echo ($vhsncafc['Block']['name']) ? $vhsncafc['Block']['name']: 'All Block'; ?></td>
<td><?php //echo h(ucfirst($vhsncafc['Panchayat']['name'])); 
echo ($vhsncafc['Panchayat']['name']) ? $vhsncafc['Panchayat']['name']: 'All Panchayat';
?></td>
...
<td><?php  echo h(ucfirst($vhsncafc['Commonreport']['activity']));  ?></td>
<td><?php  echo h(ucfirst($vhsncafc['User']['name'].' '.$vhsncafc['User']['last_name']));  ?></td>
<td class="actions">
<a href="javascript:void(0)" class="more btn btn-success" data-toggle="modal" data-target="#myModal" data-id="<?=$vhsncafc['Commonreport']['date']?>,<?=$vhsncafc['Commonreport']['panchayat']?>">More</a>
<?php /*?><?php 
...
$.ajax({url:"<?=SITE_PATH?>commonreports/viewactivity/"+dataid,dataType:"html",success:function(result){
...
$.ajax({url:"<?=SITE_PATH?>commonreports/getusers/"+c,success:function(result){$("#user").html(result);}});
...
var c=$(this).val();
$('#user').html("<option value=''>loading......</option>"); 
$.ajax({url:"<?=SITE_PATH?>commonreports/getpusers/"+c,success:function(result){$("#user").html(result);}});
