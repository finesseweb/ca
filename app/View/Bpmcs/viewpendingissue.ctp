...
<div class="actions"><h2><?php echo __('BPMC Meeting Details'); ?></h2></div>
...
<div class="btn-group">
<?php echo $this->Html->link(__('New BPMC Meeting'), array('action' => 'add'),array('class' => 'btn btn-primary')); ?>
...
<div class="row">
<form id="mastersearch" mathod="get" action="<?=SITE_PATH?>bpmcs/">
<?php /*?> <div class="txt_one">SEARCH</div><?php */?>
<div class="col-sm-3"><input type="text" name="search_key" placeholder="By Decisions taken,Issue resolved,Issues details " class="form-control" value="<? if(isset($this->params->query['search_key'])){ echo trim($this->params->query['search_key']); }?>"/></div>
...
<div class="col-sm-1"><input type="button" name="reset" value="Reset" class="btn btn-warning btn-block" onclick="window.location.href='<?=SITE_PATH?>bpmcs/'"/></div>
<div class="col-sm-1" style="float:right;"><input type="button" name="reset" value="Export" class="btn btn-block export" onclick="window.open('<?=SITE_PATH?>bpmcs/export/?<?=$_SERVER['QUERY_STRING']?>','abhay','scrollbars=1,width=1400,height=650,left=100,top=50')"/></div>
...
<th><?php //echo $this->Paginator->sort(''); ?>Issues shared in BPMC</th>
<th><?php //echo $this->Paginator->sort('Meeting Facilitated by'); ?>Decision taken</th>
<th><?php //echo $this->Paginator->sort(''); ?>Issues resolved</th>
<th><?php //echo $this->Paginator->sort('Meeting Facilitated by'); ?>Issues forwarded</th>
...
<tr>
<td><?php echo h($anm['Bpmc']['id']); ?></td>
<td><?php echo h(ucfirst($anm['Block']['name'])); ?></td>
<td><?php echo h(date('d-m-Y',strtotime($anm['Bpmc']['meeting_date']))); ?></td>
<td><?php echo h(ucfirst($anm['Bpmc']['issue_shared_bpmc'])); ?></td>
<td><?php echo ucwords($anm['Bpmc']['decisions_taken']);?></td>
<td><?php echo ucwords($anm['Bpmc']['issue_resolved']);?></td>
<td><?php echo h(ucfirst($anm['Bpmc']['letter_to_higher_authority'])); ?></td>


<td class="actions">
<a href="javascript:void(0)" class="more btn btn-success" data-toggle="modal" data-target="#myModal" data-id="<?=$anm['Bpmc']['id']?>">More</a>
<?php 
if($sessionval=='regular') {
?>
 <?php
if(in_array($this->request->params['controller'].":edit",$menu) && $anm['Bpmc']['updated']==0){
...
$.ajax({url:"<?=SITE_PATH?>/bpmcs/view/"+dataid,dataType:"html",success:function(result){
