...
<div class="actions"><h2><?php echo __('Adolescent Meeting Details'); ?></h2></div>
<?php $menu= $this->Session->read('User.mainmenu');
      $sessionval=$this->Session->read('User.type');
      
?>
<div class="btn-group">
<?php echo $this->Html->link(__('New Adolescent Meeting'), array('action' => 'add'),array('class' => 'btn btn-primary')); ?>
...
<div class="row">
<form id="mastersearch" mathod="get" action="<?=SITE_PATH?>adolescentMeetings/">
<?php /*?> <div class="txt_one">SEARCH</div><?php */?>
<div class="col-sm-3"><input type="text" name="search_key" placeholder="By Group Name,Total Members" class="form-control" value="<? if(isset($this->params->query['search_key'])){ echo trim($this->params->query['search_key']); }?>"/></div>
...
<div class="col-sm-1"><input type="button" name="reset" value="Reset" class="btn btn-warning btn-block" onclick="window.location.href='<?=SITE_PATH?>adolescentMeetings/'"/></div>
<div class="col-sm-1" style="float:right; margin-top:15px;"><input type="button" name="reset" value="Export" class="btn btn-block export" onclick="window.open('<?=SITE_PATH?>adolescentMeetings/export/?<?=$_SERVER['QUERY_STRING']?>','abhay','scrollbars=1,width=1400,height=650,left=100,top=50')"/></div>
...
<th><?php ///echo $this->Paginator->sort(''); ?>Total Participants </th>

<th><?php echo $this->Paginator->sort('topic_discussed'); ?></th>

<?php /*?> <th><?php echo $this->Paginator->sort('B.form'); ?></th><?php */?>
<th class="actions"><?php echo __('Actions'); ?></th>
</tr>
<?php foreach ($adolescentmeetings as $adolescentmeeting): ?>
<tr>
<td><?php echo h($adolescentmeeting['AdolescentMeeting']['id']); ?></td>
<td><?php echo h($adolescentmeeting['Block']['name']); ?></td>
<td><?php echo h(ucfirst($adolescentmeeting['Panchayat']['name'])); ?></td>
<td><?php echo h(ucfirst($adolescentmeeting['Village']['name']) ? $adolescentmeeting['Village']['name']: 'All Village'); ?></td>
<td><?php echo h(date('d-m-Y',strtotime($adolescentmeeting['AdolescentMeeting']['date']))); ?></td>
<td><?php echo h(ucfirst($adolescentmeeting['Youthleader']['group_name'])); ?></td>
<td><?php echo h(ucfirst($adolescentmeeting['AdolescentMeeting']['no_of_participants'])); ?></td>
<td><?php echo h(ucfirst($adolescentmeeting['Discussion']['name'])); ?></td>
<td class="actions">
<a href="javascript:void(0)" class="more btn btn-success" data-toggle="modal" data-target="#myModal" data-id="<?=$adolescentmeeting['AdolescentMeeting']['id']?>">More</a>
<?php 
...
?>
<?php echo $this->Html->link(__('Edit'), array('action' => 'edit', $adolescentmeeting['AdolescentMeeting']['id']),array('class' => 'btn btn-primary')); ?>
<?php } ?>
   <?php
if(in_array($this->request->params['controller'].":delete",$menu)){
  
?>
  <?php echo $this->Form->postLink(__('Delete'), array('action' => 'delete', $adolescentmeeting['AdolescentMeeting']['id']),array('class' => 'btn btn-danger'), null, __('Are you sure you want to delete # %s?', $adolescentmeeting['AdolescentMeeting']['id'])); ?>
...
$.ajax({url:"<?=SITE_PATH?>adolescentMeetings/view/"+dataid,dataType:"html",success:function(result){
