...
<div class="actions"><h2><?php echo __('ANM Meeting Details'); ?></h2></div>
...
<div class="btn-group">
<?php echo $this->Html->link(__('New ANM Meeting'), array('action' => 'add'),array('class' => 'btn btn-primary')); ?>
...
<div class="row">
<form id="mastersearch" mathod="get" action="<?=SITE_PATH?>anmmeetings/">
<?php /*?> <div class="txt_one">SEARCH</div><?php */?>
<div class="col-sm-3"><input type="text" name="search_key" placeholder="By Meeting Chaired,Decisions taken,Issue resolved" class="form-control" value="<? if(isset($this->params->query['search_key'])){ echo trim($this->params->query['search_key']); }?>"/></div>
...
<div class="col-sm-1"><input type="button" name="reset" value="Reset" class="btn btn-warning btn-block" onclick="window.location.href='<?=SITE_PATH?>anmmeetings/'"/></div>
<div class="col-sm-1" style="float:right;"><input type="button" name="reset" value="Export" class="btn btn-block export" onclick="window.open('<?=SITE_PATH?>anmmeetings/export/?<?=$_SERVER['QUERY_STRING']?>','abhay','scrollbars=1,width=1400,height=650,left=100,top=50')"/></div>
...
<th><?php echo $this->Paginator->sort('key_issues_discussed'); ?></th>
<th><?php ///////////echo $this->Paginator->sort('issue raised by'); ?>Issue raised by</th>
<th><?php ///////////echo $this->Paginator->sort('issue raised by'); ?>Decision Taken</th>
<th><?php ///////////echo $this->Paginator->sort('issue raised by'); ?>Issues Resolved</th>
...
<tr>
<td><?php echo h($anm['AnmMeeting']['id']); ?></td>
<td><?php echo h(ucfirst($anm['Block']['name'])); ?></td>
<td><?php echo h(date('d-m-Y',strtotime($anm['AnmMeeting']['meeting_date']))); ?></td>
<td><?php echo h(ucfirst($anm['AnmMeeting']['key_issues_discussed'])); ?></td>
<td><?php echo h(ucfirst($anm['AnmMeeting']['issues_raised_by_bpc'])); ?></td>
<td><?php echo h(ucfirst($anm['AnmMeeting']['decisions_taken'])); ?></td>
<td><?php echo h(ucfirst($anm['AnmMeeting']['issue_resolved'])); ?></td>
<td class="actions">
<a href="javascript:void(0)" class="more btn btn-success" data-toggle="modal" data-target="#myModal" data-id="<?=$anm['AnmMeeting']['id']?>">More</a>
<?php 
...
$.ajax({url:"<?=SITE_PATH?>/anmmeetings/view/"+dataid,dataType:"html",success:function(result){
