...
<div class="actions"><h2><?php echo __('VHSNC Untied Funds Details'); ?></h2></div>
<?php $menu= $this->Session->read('User.mainmenu');
      $sessionval=$this->Session->read('User.type');
      
?>
<div class="btn-group">
<?php echo $this->Html->link(__('New VHSNC Untied Fund'), array('action' => 'add'),array('class' => 'btn btn-primary')); ?>
<?php echo $this->Html->link(__('List of Expenditure'), array('controller' => 'vhsncUntiedfunds','action' => 'index'),array('class' => 'btn btn-primary')); ?>
 <?php echo $this->Html->link(__('Add Expenditure'), array('controller' => 'vhsncUntiedfunds','action' => 'add'),array('class' => 'btn btn-primary')); ?>
</div>

<div class="panel panel-default">
<div class="panel-body">
<div class="row">
<form id="mastersearch" mathod="get" action="<?=SITE_PATH?>vhsncUntiedfundDetails/">
<?php /*?> <div class="txt_one">SEARCH</div><?php */?>
<div class="col-sm-3"><input type="text" name="search_key" placeholder="By Bank A/C,IFSC,VHSNC Fund Recieved," class="form-control" value="<? if(isset($this->params->query['search_key'])){ echo trim($this->params->query['search_key']); }?>"/></div>
...
<div class="col-sm-1"><input type="button" name="reset" value="Reset" class="btn btn-warning btn-block" onclick="window.location.href='<?=SITE_PATH?>vhsncUntiedfundDetails/'"/></div>
<div class="col-sm-1" style="float:right; margin-top:15px;"><input type="button" name="reset" value="Export" class="btn btn-block export" onclick="window.open('<?=SITE_PATH?>vhsncUntiedfundDetails/export/?<?=$_SERVER['QUERY_STRING']?>','abhay','scrollbars=1,width=1400,height=650,left=100,top=50')"/></div>
...
<th><?php echo $this->Paginator->sort('vhsnc_name'); ?></th>
<th><?php echo $this->Paginator->sort('bank_account_number'); ?></th>
<th><?php //echo $this->Paginator->sort('Financial years'); ?>Financial years</th>
<th><?php echo $this->Paginator->sort('vhsnc_funds_recieved'); ?></th>
<th class="actions"><?php echo __('Actions'); ?></th>
</tr>
<?php foreach ($untiedfunds as $untiedfund): ?>
<tr>
<td><?php echo h($untiedfund['VhsncUntiedfundDetail']['id']); ?></td>
...
<td><?php echo h(ucfirst($untiedfund['VhsncConstitution']['vhsnc_name'])); ?></td>
<td><?php echo h(ucfirst($untiedfund['VhsncUntiedfundDetail']['bank_account_number'])); ?></td>
<td><?php echo h(date('d-m-Y',strtotime($untiedfund['ReportingPeriod']['from_date']))); ?> To <?php echo h(date('d-m-Y',strtotime($untiedfund['ReportingPeriod']['to_date']))); ?></td>
<td><?php echo h(ucfirst($untiedfund['VhsncUntiedfundDetail']['vhsnc_funds_recieved'])); ?></td>
<td class="actions">
<a href="javascript:void(0)" class="more btn btn-success" data-toggle="modal" data-target="#myModal" data-id="<?=$untiedfund['VhsncUntiedfundDetail']['id']?>">More</a>
<?php 
...
?>
 <?php echo $this->Html->link(__('Edit'), array('action' => 'edit', $untiedfund['VhsncUntiedfundDetail']['id']),array('class' => 'btn btn-primary')); ?>
<?php } ?>
   <?php
if(in_array($this->request->params['controller'].":delete",$menu)){
  
?>
  <?php echo $this->Form->postLink(__('Delete'), array('action' => 'delete', $untiedfund['VhsncUntiedfundDetail']['id']),array('class' => 'btn btn-danger'), null, __('Are you sure you want to delete # %s?', $untiedfund['VhsncUntiedfundDetail']['id'])); ?>
...
$.ajax({url:"<?=SITE_PATH?>vhsncUntiedfundDetails/view/"+dataid,dataType:"html",success:function(result){
