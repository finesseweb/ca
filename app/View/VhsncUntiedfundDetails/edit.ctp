...
<?php foreach ($reporting_periods as $key=>$value){
    //print_r($value);
    ?>
<option value="<?=$value['reporting_periods']['id']?>" <?php if($this->request->data['VhsncUntiedfundDetail']['financial_year']==$value['reporting_periods']['id']) { echo "selected"; }?>><?=date('d-m-Y',strtotime($value['reporting_periods']['from_date']))?> To <?=date('d-m-Y',strtotime($value['reporting_periods']['to_date']))?></option>
<?php } ?>
</select></div></div>
    <?php 
