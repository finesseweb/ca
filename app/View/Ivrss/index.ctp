...
<div class="actions"><h2><?php echo __('M-Shakti Details'); ?></h2></div>
<?php $menu= $this->Session->read('User.mainmenu');
      $sessionval=$this->Session->read('User.type');
      
?>
<div class="btn-group">
<?php echo $this->Html->link(__('New M-Shakti'), array('action' => 'add'),array('class' => 'btn btn-primary')); ?>
...
<div class="row">
<form id="mastersearch" mathod="get" action="<?=SITE_PATH?>Ivrss/">
<?php /*?> <div class="txt_one">SEARCH</div><?php */?>
<div class="col-sm-3"><input type="text" name="search_key" placeholder="By User Name,Mobile,User type,Gender" class="form-control" value="<? if(isset($this->params->query['search_key'])){ echo trim($this->params->query['search_key']); }?>"/></div>
...
<div class="col-sm-1"><input type="button" name="reset" value="Reset" class="btn btn-warning btn-block" onclick="window.location.href='<?=SITE_PATH?>Ivrss/'"/></div>
<div class="col-sm-1" style="float:right; margin-top:15px;"><input type="button" name="reset" value="Export" class="btn btn-block export" onclick="window.open('<?=SITE_PATH?>Ivrss/export/?<?=$_SERVER['QUERY_STRING']?>','abhay','scrollbars=1,width=1400,height=650,left=100,top=50')"/></div>
...
<th><?php echo $this->Paginator->sort('ivrs_user_name'); ?></th>
<th><?php echo $this->Paginator->sort('ivrs_user_mobile'); ?></th>


<?php /*?> <th><?php echo $this->Paginator->sort('B.form'); ?></th><?php */?>
<th class="actions"><?php echo __('Actions'); ?></th>
</tr>
<?php foreach ($Ivrss as $Ivrs): ?>
<tr>
<td><?php echo h($Ivrs['Ivrs']['id']); ?></td>
<td><?php echo h(ucfirst($Ivrs['Block']['name'])); ?></td>
<td><?php echo h($Ivrs['Panchayat']['name']); ?></td>
<td><?php echo h(ucfirst($Ivrs['Village']['name'])? $Ivrs['Village']['name']: 'All Village'); ?></td>
<td><?php echo h(date('d-m-Y', strtotime($Ivrs['Ivrs']['date']))); ?></td>
<td><?php echo h(ucfirst($Ivrs['Ivrs']['ivrs_user_name'])); ?></td>
<td><?php echo h(ucfirst($Ivrs['Ivrs']['ivrs_user_mobile'])); ?></td>

<td class="actions">
<a href="javascript:void(0)" class="more btn btn-success" data-toggle="modal" data-target="#myModal" data-id="<?=$Ivrs['Ivrs']['id']?>">More</a>
<?php 
...
?>
<?php echo $this->Html->link(__('Edit'), array('action' => 'edit', $Ivrs['Ivrs']['id']),array('class' => 'btn btn-primary')); ?>
<?php } ?>
   <?php
if(in_array($this->request->params['controller'].":delete",$menu)){
  
?>
  <?php echo $this->Form->postLink(__('Delete'), array('action' => 'delete', $Ivrs['Ivrs']['id']),array('class' => 'btn btn-danger'), null, __('Are you sure you want to delete # %s?', $Ivrs['Ivrs']['id'])); ?>
...
$.ajax({url:"<?=SITE_PATH?>Ivrss/view/"+dataid,dataType:"html",success:function(result){
