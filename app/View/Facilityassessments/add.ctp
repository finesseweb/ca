...
echo "<div class='col-sm-3'>".$this->Form->input('invsetigation_date',array('type'=>'text','class'=>'calbsp tform form-control','label'=>'Date of Invsetigation','value'=>date('d-m-Y'),'readonly'=>'readonly'))."</div>";
    ...
    <table class="table table-striped"> 
        ...
        <tbody>
             <?php foreach($feedbacks as $feedback) {
                 $class=$this->requestAction(array("controller"=>"subfeedbacks","action"=>"getclass",$feedback['Feedback']['id'])); 
                              ?>
          <tr>
              <td class="<?=$class['Subfeedback']['facility_level']?>" colspan="3" style="background-color: #C0BFBA;"><?=ucwords($feedback['Feedback']['name'])?>
              ...
              <?php  $count=0;
                 ...
                 ?>
          <tr class="<?=$value['Subfeedback']['facility_level']?>"><td style="background-color: aliceblue" class="question" id="feedId<?=ucfirst($value['Subfeedback']['id'])?>">
...
<!--                  <input class="form-control question" type="textarea" name="data[FacilityAssessment][question][]" id="feedId<?=ucfirst($value['Subfeedback']['id'])?>" readonly="readonly" value="<?=ucfirst($value['Subfeedback']['name'])?>">-->
              ...
              <td style="width: 14%"> <select class="form-control question" name="data[FacilityAssessment][response][]" id="responce<?=ucfirst($value['Subfeedback']['id'])?>">
                  ...
                  <?php } ?>
                   <?php if($value['Subfeedback']['responce_four']!=''){?>
                  <option value="<?=$value['Subfeedback']['responce_four'];?>"> <?=$value['Subfeedback']['responce_four'];?> </option>
                  <?php } ?>
              </select>
          </td>
...
//    });
 ...
 $("#FacilityAssessmentFacilityLevel").change(function(){
var c=$(this).val();
//alert(c);
if(c==='hsc'){
    $('.hsc').show();
    ...
    if (c.length<10)
           {
            ...
            alert("Enter max 10 digit");
                $( "#FacilityAssessmentMobileResponderTwo" ).val('');
                return false;  
