...
<div class="actions"><h2><?php echo __('VHSNC Member Details'); ?></h2></div>
<?php $menu= $this->Session->read('User.mainmenu');
      $sessionval=$this->Session->read('User.type');
      
?>
<div class="btn-group">
<?php echo $this->Html->link(__('New VHSNC Member '), array('action' => 'add'),array('class' => 'btn btn-primary')); ?>
<?php //echo $this->Html->link(__('List VHSNC Constitutions'), array('controller' => 'vhsncConstitutions','action' => 'index'),array('class' => 'btn btn-primary')); ?>
</div>

<div class="panel panel-default">
<div class="panel-body">
<div class="row">
<form id="mastersearch" mathod="get" action="<?=SITE_PATH?>vhsncMembers/">
<?php /*?> <div class="txt_one">SEARCH</div><?php */?>
<div class="col-sm-3"><input type="text" name="search_key" placeholder="By Mobile,Status" class="form-control" value="<? if(isset($this->params->query['search_key'])){ echo trim($this->params->query['search_key']); }?>"/></div>
...
<div class="col-sm-1"><input type="button" name="reset" value="Reset" class="btn btn-warning btn-block" onclick="window.location.href='<?=SITE_PATH?>vhsncMembers/'"/></div>
<div class="col-sm-1" style="float:right; margin-top:15px;"><input type="button" name="reset" value="Export" class="btn btn-block export" onclick="window.open('<?=SITE_PATH?>vhsncMembers/export/?<?=$_SERVER['QUERY_STRING']?>','abhay','scrollbars=1,width=1400,height=650,left=100,top=50')"/></div>
...
<th><?php echo $this->Paginator->sort('vHSNC_name'); ?> (Click on name to View member)</th>
<th><?php //echo $this->Paginator->sort('total_members'); ?>Total members</th>
...
<td><a href="javascript:void(0)" data-toggle="modal" class="moremembres" data-target="#myModal" data-id="<?=$vhsnc['VhsncMember']['vhsnc_name']?>"><?php echo h(ucfirst($vhsnc['VhsncConstitution']['vhsnc_name'])); ?></a></td>
<td><?php 
$questionlist=$this->requestAction(array("controller"=>"vhsncMembers","action"=>"getall",$vhsnc['VhsncMember']['vhsnc_name'])); 
                 //print_r($questionlist);
                  echo $questionlist;

//echo h(ucfirst($vhsnc['VhsncConstitution']['total_members'])); ?></td>
<td class="actions">
<a href="javascript:void(0)" class="moremembres btn btn-success" data-toggle="modal" data-target="#myModal" data-id="<?=$vhsnc['VhsncMember']['vhsnc_name']?>">More</a>
</td>
...
$.ajax({url:"<?=SITE_PATH?>vhsncMembers/view/"+dataid,dataType:"html",success:function(result){
if(result!=''){$("#resalemore").html(result);} else {$("#resalemore").html("<div class='loading'>Data not found</div>");}
}});
});

$(".moremembres").click(function(){
$('#resultTable tr').removeClass("done");
$(this).parent().parent().addClass("done");
var dataid=$(this).attr('data-id');
$('.right_resale').html("<div class='loading'><img src='<?=SITE_PATH?>images/loader.gif'> Please wait .Loading........</div>");
$.ajax({url:"<?=SITE_PATH?>vhsncMembers/viewmembers/"+dataid,dataType:"html",success:function(result){
if(result!=''){$("#resalemore").html(result);} else {$("#resalemore").html("<div class='loading'>Data not found</div>");}
}});
});
