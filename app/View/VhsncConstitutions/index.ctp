...
<div class="actions"><h2><?php echo __('VHSNC Constitutions Details'); ?></h2></div>
<?php $menu= $this->Session->read('User.mainmenu');
      $sessionval=$this->Session->read('User.type');
      
?>
<div class="btn-group">
<?php echo $this->Html->link(__('New VHSNC Constitutions'), array('action' => 'add'),array('class' => 'btn btn-primary')); ?>
<?php echo $this->Html->link(__('List Member VHSNC'), array('controller' => 'vhsncMembers','action' => 'index'),array('class' => 'btn btn-primary')); ?>
<?php echo $this->Html->link(__('Add Member Into VHSNC'), array('controller' => 'vhsncMembers','action' => 'add'),array('class' => 'btn btn-primary')); ?>
...
<div class="row">
<form id="mastersearch" mathod="get" action="<?=SITE_PATH?>vhsncConstitutions/">
<?php /*?> <div class="txt_one">SEARCH</div><?php */?>
<div class="col-sm-3"><input type="text" name="search_key" placeholder="By VHSNC Name,VHSNC Constitution level,Signatory" class="form-control" value="<? if(isset($this->params->query['search_key'])){ echo trim($this->params->query['search_key']); }?>"/></div>
...
<div class="col-sm-1"><input type="button" name="reset" value="Reset" class="btn btn-warning btn-block" onclick="window.location.href='<?=SITE_PATH?>vhsncConstitutions/'"/></div>
<div class="col-sm-1" style="float:right; margin-top:15px;"><input type="button" name="reset" value="Export" class="btn btn-block export" onclick="window.open('<?=SITE_PATH?>vhsncConstitutions/export/?<?=$_SERVER['QUERY_STRING']?>','abhay','scrollbars=1,width=1400,height=650,left=100,top=50')"/></div>
...
<th><?php //echo $this->Paginator->sort('vHSNC_constitution_name'); ?>VHSNC Constitution Name</th>
<th><?php echo $this->Paginator->sort('constitution_date'); ?></th>
<th><?php //echo $this->Paginator->sort('vHSNC_account_number'); ?>VHSNC Account number</th>
<th><?php echo $this->Paginator->sort('primary_signatory'); ?></th>
<th><?php echo $this->Paginator->sort('secondary_signatory'); ?></th>
<th class="actions"><?php echo __('Actions'); ?></th>
</tr>
<?php foreach ($vhsncs as $vhsnc): ?>
<tr>
<td><?php echo h($vhsnc['VhsncConstitution']['id']); ?></td>
<td><?php echo h(ucfirst($vhsnc['City']['name'])); ?></td>
<td><?php echo h(ucfirst($vhsnc['Block']['name'])); ?></td>
<td><?php echo h(ucfirst($vhsnc['Panchayat']['name'])); ?></td>
<td><?php echo h(ucfirst($vhsnc['VhsncConstitution']['vhsnc_name'])); ?></td>
<td><?php echo h(date('d-m-Y',strtotime($vhsnc['VhsncConstitution']['constitution_date']))); ?></td>
<td><?php echo h(ucfirst($vhsnc['VhsncConstitution']['account_no'])); ?></td>
<td><?php echo h(ucfirst($vhsnc['VhsncConstitution']['primary_signatory'])); ?></td>
<td><?php echo h(ucfirst($vhsnc['VhsncConstitution']['secondary_signatory'])); ?></td>
<td class="actions">
<a href="javascript:void(0)" class="more btn btn-success" data-toggle="modal" data-target="#myModal" data-id="<?=$vhsnc['VhsncConstitution']['id']?>">More</a>
<?php 
...
?>
<?php echo $this->Html->link(__('Edit'), array('action' => 'edit', $vhsnc['VhsncConstitution']['id']),array('class' => 'btn btn-primary')); ?>
<?php } ?>
   <?php
if(in_array($this->request->params['controller'].":delete",$menu)){
  
?>
 <?php echo $this->Form->postLink(__('Delete'), array('action' => 'delete', $vhsnc['VhsncConstitution']['id']),array('class' => 'btn btn-danger'), null, __('Are you sure you want to delete # %s?', $vhsnc['VhsncConstitution']['id'])); ?>
...
$.ajax({url:"<?=SITE_PATH?>vhsncConstitutions/view/"+dataid,dataType:"html",success:function(result){
