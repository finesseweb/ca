 ...
 */
    ...
    var  $uses = array('Administration','User','Project','Ngo','Location','Country','City','Block','Designation');
/**
 * index method
 *
 * @return void
 */
    ...
    $condition['OR']=array('Administration.id LIKE'=>'%'.$searchKey.'%','Administration.first_name LIKE'=>'%'.$searchKey.'%','Administration.last_name LIKE'=>'%'.$searchKey.'%','Administration.mobile LIKE '=>'%'.$searchKey.'%','Administration.emailid LIKE '=>'%'.$searchKey.'%'); 
...
//		
	    ...
	    $this->Paginator->settings = array('Administration' => array('limit' =>20,'order' => array('id' => 'desc'),'conditions'=>$condition));
 ...
 */
	public function edit($id = null) {
		...
		if ($this->request->is(array('post', 'put'))) {
			...
			$executives=$this->User->find('all',array('conditions'=>array('User.status'=>'active','User.role'=>'regular','User.id'=>$this->request->data['Administration']['first_name'])));
			if($this->request->data['Administration']['organization']!=0 and $this->request->data['Administration']['organization']!='')
			{
				$ngos=$this->Ngo->find('list',array('conditions'=>array('Ngo.id'=>$this->request->data['Administration']['organization'])));
				}
			else
			{
				$ngos=$this->Ngo->find('list');
                                ...
                                if($this->request->data['Administration']['designation']!=0 and $this->request->data['Administration']['designation']!='')
			{
				$desig=$this->Designation->find('list',array('conditions'=>array('Designation.id'=>$this->request->data['Administration']['designation'])));
				}
			else
			{
				$desig=$this->Designation->find('list');
 ...
 */
	public function delete($id = null,$status='deactive') {
		...
		if (!$this->Administration->exists()) {
		    ...
		    $this->Administration->read(null,$id);
			$this->Administration->set(array('status'=>$status));
		
		if ($this->Administration->save()) {
