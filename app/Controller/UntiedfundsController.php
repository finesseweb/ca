 ...
 */
class UntiedfundsController extends AppController {
 ...
 */
    ...
    var  $uses = array('Untiedfund','Geographical','Ngo','User','Project','Village','Panchayat','Location','Country','City','Block','Designation','ReportingPeriod');
/**
 * index method
 *
 * @return void
 */
    ...
    $condition['OR']=array('Untiedfund.id LIKE'=>'%'.$searchKey.'%','Untiedfund.cc_name LIKE'=>'%'.$searchKey.'%','Untiedfund.bank_name LIKE'=>'%'.$searchKey.'%','Untiedfund.ifsc LIKE '=>'%'.$searchKey.'%'); 
...
//		
		...
		$this->Paginator->settings = array('Untiedfund' => array('limit' =>20,'order' => array('id' => 'desc'),'conditions'=>$condition));
		$this->Untiedfund->recursive = 0;
 ...
 */
	public function view($id = null) {
		if (!$this->Untiedfund->exists($id)) {
			throw new NotFoundException(__('Invalid Untiedfund'));
		}
		$options = array('conditions' => array('Untiedfund.' . $this->Untiedfund->primaryKey => $id));
		$this->set('untiedfund', $this->Untiedfund->find('first', $options));
 ...
 */
	public function add() {
		...
		if ($this->request->is('post')) {
			$this->Untiedfund->create();
                        //print_r($this->request->data);
                        //die();
                          $organization =  $this->request->data['Untiedfund']['organization'];
                          $district =  $this->request->data['Untiedfund']['district'];
                          $block =  $this->request->data['Untiedfund']['block'];
                          $panchayat =  $this->request->data['Untiedfund']['panchayat'];
                          $cc_name =  $this->request->data['Untiedfund']['cc_name'];
                          $member_num=  $this->request->data['Untiedfund']['member_num'];
                          $constitution_date =  date('Y-m-d',strtotime($this->request->data['Untiedfund']['constitution_date']));
                          $bank_account =  $this->request->data['Untiedfund']['bank_account'];
                          $bank_name=  $this->request->data['Untiedfund']['bank_name'];
                          $ifsc =  $this->request->data['Untiedfund']['ifsc'];
                          $untied_funds_received =  $this->request->data['Untiedfund']['untied_funds_received'];
                          $financial_year =  $this->request->data['Untiedfund']['financial_year'];
                          $status =  $this->request->data['Untiedfund']['status'];
		...
		$this->set(compact('panchayat','cities','ngos','blocks','desig','village','reporting_periods'));
	}
 ...
 */
	public function edit($id = null) {
		...
		if ($this->request->is(array('post', 'put'))) {
			...
			$options = array('conditions' => array('Untiedfund.' . $this->Untiedfund->primaryKey => $id));
			$this->request->data = $this->Untiedfund->find('first', $options);
                        
//                  if($this->request->data['Untiedfund']['financial_year']!=0 and $this->request->data['Untiedfund']['financial_year']!='')
//		{
//		$reporting_periods=$this->ReportingPeriod->query("select * from reporting_periods where id =".$this->request->data['Untiedfund']['financial_year']);
//	       }
//		else
//		{
//		$reporting_periods=$this->ReportingPeriod->query("select * from reporting_periods");
//		}
	...
	public function editDetails($id = null) {
		if (!$this->Ngo->exists($id)) {
			throw new NotFoundException(__('Invalid booking'));
		}
		if ($this->request->is(array('post', 'put'))) {
			if ($this->Ngo->save($this->request->data)) {
				$this->Session->setFlash(__('The Geographical has been saved.'));
				return $this->redirect(array('action' => 'index'));
			} else {
				$this->Session->setFlash(__('The Geographical could not be saved. Please, try again.'));
			}
		} else {
			$options = array('conditions' => array('Ngo.' . $this->Ngo->primaryKey => $id));
			$this->request->data = $this->Ngo->find('first', $options);
			$executives=$this->User->find('list',array('conditions'=>array('User.status'=>'active','User.role'=>'regular')));
			if($this->request->data['Ngo']['bulider_name']!=0 and $this->request->data['Ngo']['bulider_name']!='')
			{
				$projects=$this->Project->find('list',array('conditions'=>array('Project.builder_id'=>$this->request->data['Ngo']['bulider_name'])));
				}
			else
			{
	...
	public function bookingDetails($id = null) {
		...
		$options = array('conditions' => array('Ngo.' . $this->Ngo->primaryKey => $id));
		$this->set('booking', $this->Ngo->find('first', $options));
		$this->layout='newdefault';
	}
	...
	public function bookingForm($id=null) {
		...
		if ($this->request->is(array('post', 'put'))) {
			...
			$resultval="" ;
			if($this->request->data['Ngo']['booking_pdf_new']['size']!=0){
			if(file_exists($destinationorig.$this->request->data['Ngo']['booking_pdf']))	{
			@unlink($destinationorig.$this->request->data['Ngo']['booking_pdf']);
			}
			
			$file = $this->request->data['Ngo']['booking_pdf_new'];
			$resultval = $this->Upload->uploadimg($file,$destinationorig,'','',''); 
			$this->request->data['Ngo']['booking_pdf']=$resultval;
			
			
			$this->Ngo->read(null,$id);
			$this->Ngo->set(array('booking_pdf'=>$this->request->data['Ngo']['booking_pdf']));
			$this->Ngo->save();
			...
			$options = array('fields'=>array('id','booking_pdf'),'conditions' => array('Ngo.' . $this->Ngo->primaryKey => $id));
			$this->request->data = $this->Ngo->find('first', $options);
			//$this->set('booking', $this->Ngo->find('first', $options));
		}
		$this->layout='ajax';
 ...
 */
	public function delete($id = null,$status='deactive') {
		$this->Untiedfund->id = $id;
		if (!$this->Untiedfund->exists()) {
			throw new NotFoundException(__('Invalid Facility Detail'));
		}
		    //$this->request->onlyAllow('post', 'delete');
		    $this->Untiedfund->read(null,$id);
			$this->Untiedfund->set(array('status'=>$status));
		
		if ($this->Untiedfund->save()) {
...
if($val!="")
	...
	$condition['Ngo.project_name']=$this->params->query['subcategory'];
	}
	...
	$condition['AND']=array('Ngo.booked_by'=>$this->params->query['category'],'Ngo.booked_by_2'=>$this->params->query['category']);
	}
	elseif($this->params->query['type']=="location")
	{
	$query.=" ( project_location='".$this->params->query['category']."' ) and ";
	$condition['Ngo.project_location']=$this->params->query['category'];
	}
	elseif($this->params->query['type']=="cheque")
	{
	$query.=" ( cheque_clearance_status='".$this->params->query['category']."' ) and ";
	$condition['Ngo.cheque_clearance_status']=$this->params->query['category'];
	}
	elseif($this->params->query['type']=="bookingstatus")
	{
	$query.=" ( booking_canceled='".trim($this->params->query['category'])."' )  and ";
	$condition['Ngo.booking_canceled']=$this->params->query['category'];
	}
	$query.=" ( date(date_of_booking) between '".$startdate."' and '".$enddate."') and ( status='active' ) order by date_of_booking ";
	$condition['AND']=array('date(Ngo.date_of_booking) >='=>$startdate,'date(Ngo.date_of_booking) <='=>$enddate);

	//$myquery=$this->Ngo->find('all',array('conditions'=>array($condition)));
	$myquery=$this->Ngo->query($query);
	...
	$applicantdetail=$this->Ngo->query($query);
	...
	$total=$this->Ngo->query("select count(*) as total from bookings where booked_by='".$id."'  and booked_by_2='-1' and date_of_booking between '".$startdate."' and '".$enddate."'");
	...
	$total=$this->Ngo->query("select count(*) as total from bookings where ((booked_by='".$id."' and booked_by_2!='-1') or (booked_by!='-1' and booked_by_2='".$id."')) and date_of_booking between '".$startdate."' and '".$enddate."'");
	return $total[0][0]['total'];
    }
	...
	$total=$this->Ngo->query("select distinct( build.name),build.id From builders as build where build.status='active' order by build.name asc");
	return $total;
    }
...
$total=$this->Ngo->query("select count(*) as total from bookings where bulider_name='".$builder."' and date_of_booking between '".$startdate."' and '".$enddate."'");
	return $total[0][0]['total'];
	...
	$total=$this->Ngo->query("select distinct book.project_location,loc.name from bookings as book join locations as loc on book.project_location=loc.id where status='active' and date_of_booking between '".$startdate."' and '".$enddate."' order by loc.name asc");
	return $total;
	...
	$total=$this->Ngo->query("select distinct loc.name,loc.id from locations as loc where status='active' order by loc.name asc");
	return $total;
    }
	...
	$total=$this->Ngo->query("select count(*) as total from bookings where status='active' and project_location='".$location."' and date_of_booking between '".$startdate."' and '".$enddate."'");
	...
	$total=$this->Ngo->query("select * from users where role='regular'   and status='active'  order by name asc");
	return $total;
    }
	...
	$total=$this->Ngo->query("(select distinct booked_by as user from bookings where status='active' and bookings.date_of_booking between '".$startdate."' and '".$enddate."') union (select distinct booked_by_2 as user from bookings where bookings.date_of_booking between '".$startdate."' and '".$enddate."')order by user");
	return $total;
    }
	...
	$total=$this->Ngo->query("(select Count(Distinct booked_by)  as total from bookings where bookings.date_of_booking between '".$startdate."' and '".$enddate."' and booked_by!='-1') union (select distinct booked_by_2 as user from bookings where bookings.date_of_booking between '".$startdate."' and '".$enddate."' and booked_by_2!='-1')");
	return $total[0][0]['total'];
    }
	...
	$total=$this->Ngo->query("select count(project_name) as total from bookings where status='active' and date_of_booking between '".$startdate."' and '".$enddate."'");
	return $total[0][0]['total'];
    }
	...
	$total=$this->Ngo->query("select count(distinct name) as total  from builders");
	return $total[0][0]['total'];
    }
	...
	$total=$this->Ngo->query("select count(distinct name) as total  from builders where status='active'");
	return $total[0][0]['total'];
    }
	...
	$total=$this->Ngo->query("select count(distinct project_location) as total from bookings where status='active' and date_of_booking between '".$startdate."' and '".$enddate."'");
	return $total[0][0]['total'];
    }*/
	public function countloc($startdate,$enddate) {
	$this->layout='ajax';
	...
	$total=$this->Ngo->query("select count(distinct name) as total from locations where status='active'");
	return $total[0][0]['total'];
    }
	}
